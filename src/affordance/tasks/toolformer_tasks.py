...
if dataset == "NQ" or dataset == "triviaqa":
    if dataset == "NQ":
        data_files = {split:os.path.join(cache_dir, 'nq', 'v1.0-simplified_nq-dev-all.jsonl') for split in ['train', 'dev']}
        ...
        for x in d['dev']:
            ...
            for annotation in x["annotations"]:
                ...
                for short_ans in annotation["short_answers"]:
                    answers.append(" ".join(tokens[short_ans["start_token"]:short_ans["end_token"]]))
            if len(answers):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, choices=["text-davinci-001", "text-davinci-002", "text-davinci-003", "code-davinci-002", "code-cushman-001", "davinci-codex-002-msft"], default="text-davinci-002")
